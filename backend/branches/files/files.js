...
router.get('/', async(req, res, next)=>{
    ...
    try{
        ...
        await serverHelpers.tableList(req, async (page_number, page_limit, data)=>{
            ...
            let path_inp = req.query.path ? req.query.path.replace(/\/\//g, '/') : req.query.path;
            ...
            if(!isUndefined(path_inp)) {
                ...
                data.pagination_url += data.pagination_url == '/?' ? '' : '?';
            }

            data.pagination_url += search_pagination + path_pagination;

            let content_list = await fileManager.getPathContent(file_path, backend_upload_dir, search_inp, page_number, page_limit);
            ...
            data.current_directory = path_inp;
